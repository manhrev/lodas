...
// QuerySheet chains the current query on the "sheet" edge.
func (rq *RecordQuery) QuerySheet() *SheetQuery {
	query := (&SheetClient{config: rq.config}).Query()
...
// WithSheet tells the query-builder to eager-load the nodes that are connected to
// the "sheet" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RecordQuery) WithSheet(opts ...func(*SheetQuery)) *RecordQuery {
	query := (&SheetClient{config: rq.config}).Query()
...
func (rq *RecordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Record, error) {
	...
	if query := rq.withSheet; query != nil {
		if err := rq.loadSheet(ctx, query, nodes, nil,
			func(n *Record, e *Sheet) { n.Edges.Sheet = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RecordQuery) loadSheet(ctx context.Context, query *SheetQuery, nodes []*Record, init func(*Record), assign func(*Record, *Sheet)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Record)
	for i := range nodes {
		if nodes[i].sheet_records == nil {
			continue
		}
		fk := *nodes[i].sheet_records
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sheet_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
