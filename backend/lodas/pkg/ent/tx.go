...
func (tx *Tx) init() {
	tx.BetSetting = NewBetSettingClient(tx.config)
	tx.Record = NewRecordClient(tx.config)
	tx.Result = NewResultClient(tx.config)
	tx.Sheet = NewSheetClient(tx.config)
}
...
// applies a query, for example: BetSetting.QueryXXX(), the query will be executed
